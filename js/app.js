...
const defaultState = () => ({
  ...
  mistakes:{},
  sessionSeeds:[],
  practiceStats:{quick:{runs:0,best:0},exam:{runs:0,best:0},endless:{runs:0,best:0},review:{runs:0,best:0}}
});

let state = ensureLearningState(loadState());
...
let externalQuestionBankReady = false;
...
async function loadExternalQuestionBank(){
  if(externalQuestionBankReady) return;
  try{
    const response=await fetch('./content/questions-v3.2.json',{cache:'no-store'});
    if(!response.ok) throw new Error(`HTTP ${response.status}`);
    const payload=await response.json();
    if(!Array.isArray(payload.questions)) throw new Error('Question JSON is missing a questions array.');
    const grouped={};
    for(const spec of payload.questions){
      if(!spec?.topic || !spec?.prompt || !Array.isArray(spec.options)) continue;
      const q={
        ...qChoice(spec.prompt,spec.options,Number(spec.answer),spec.skill,spec.explanation,spec.hint,spec.newbie||spec.explanation,spec.deep||spec.explanation),
        id:spec.id||'',topic:spec.topic,difficulty:Number(spec.difficulty)||1
      };
      (grouped[spec.topic] ||= []).push(q);
    }
    for(const [topic,rows] of Object.entries(grouped)) questionBank[topic]=uniqueQuestions([...(questionBank[topic]||[]),...rows]);
    externalQuestionBankReady=true;
  }catch(error){
    console.warn('External question bank could not be loaded; built-in questions remain available:',error);
  }
}

const numericQuestionGenerators={
...
function staticQuestionPoolForTopic(topic){
  if(topic==='battle-boss') return [...(questionBank.attacks||[]),...(questionBank.protocols||[]),...(questionBank.modes||[])];
  if(topic==='avalanche') return questionBank.hash||[];
  if(numericQuestionGenerators[topic]) return null;
  return questionBank[topic] || questionBank[worlds.find(w=>w.skills.includes(topic))?.skills?.[0]] || questionBank.attacks || [];
}

function buildMissionQuestions(mission,{seed=null,avoidRecent=true}={}){
  const difficulty=state.difficulty;
  const desired=mission.questions;
  const sessionSeed=seed||createSessionSeed({type:'M',id:mission.id,difficulty});
  const excluded=avoidRecent?recentQuestionKeySet(state):new Set();

  return withSeededMathRandom(sessionSeed,()=>{
    const staticPool=staticQuestionPoolForTopic(mission.topic);
    if(staticPool){
      const scale=difficultyScale(difficulty);
      const filtered=staticPool.filter(q=>!q.difficulty || Number(q.difficulty)<=Math.min(4,scale+1));
      const eligible=uniqueQuestions(filtered).length>=desired?filtered:staticPool;
      const uniquePool=uniqueQuestions(eligible);
      if(uniquePool.length<desired) console.error(`Question bank shortage for ${mission.topic}: need ${desired}, have ${uniquePool.length}.`);
      return selectUniqueStaticQuestions(uniquePool,desired,shuffle,excluded);
    }

    const generated=generateUniqueQuestions(()=>generateQuestionForTopic(mission.topic,difficulty),desired,shuffle,Math.max(500,desired*160),excluded);
    if(generated.length<desired) console.error(`Unable to generate ${desired} unique questions for ${mission.topic}; generated ${generated.length}.`);
    return generated;
  });
}

function allStaticQuestionPool(){
  const rows=[];
  for(const list of Object.values(questionBank)) rows.push(...(list||[]));
  return uniqueQuestions(rows);
}

function mixedPracticeQuestion(seed,index,targetSkill=null){
  const numericTopics=Object.keys(numericQuestionGenerators);
  const staticPool=allStaticQuestionPool();
  return withSeededMathRandom(`${seed}|${index}`,()=>{
    if(targetSkill){
      const targetedStatic=staticPool.filter(q=>q.skill===targetSkill);
      if(targetedStatic.length && Math.random()<.58) return structuredClone(sample(targetedStatic));
      const topic=numericTopics.find(t=>t===targetSkill || (t==='dh-shared'&&targetSkill==='dh') || (t==='rsa-boss'&&targetSkill==='rsa'));
      if(topic) return generateQuestionForTopic(topic,state.difficulty);
    }
    if(Math.random()<.48){
      const topic=sample(numericTopics.filter(t=>!t.endsWith('-boss')));
      return generateQuestionForTopic(topic,state.difficulty);
    }
    return structuredClone(sample(staticPool));
  });
}

function buildPracticeQuestions(count,seed,{targetSkill=null,avoidRecent=true,existingKeys=new Set()}={}){
  const excluded=avoidRecent?recentQuestionKeySet(state):new Set();
  const seen=new Set(existingKeys);
  const fresh=[]; const fallback=[];
  for(let i=0;i<count*120 && fresh.length<count;i++){
    const q=mixedPracticeQuestion(seed,i,targetSkill);
    const key=questionFingerprint(q);
    if(!key || seen.has(key)) continue;
    seen.add(key);
    if(excluded.has(key)) fallback.push(q); else fresh.push(q);
  }
  while(fresh.length<count && fallback.length) fresh.push(fallback.shift());
  return fresh.slice(0,count);
}

function generateQuestionForTopic(topic,difficulty){
  if(topic==='battle-boss') return structuredClone(sample([...(questionBank.attacks||[]),...(questionBank.protocols||[]),...(questionBank.modes||[])]));
  if(topic==='avalanche') return structuredClone(sample(questionBank.hash));
  if(numericQuestionGenerators[topic]) return numericQuestionGenerators[topic](difficulty);
...
function makeCrtQuestion(difficulty){
  const sets=difficultyScale(difficulty)>=3?[
    {r:[2,3,2],m:[3,5,7]},{r:[1,4,6],m:[5,7,11]},{r:[3,2],m:[7,11]},{r:[4,5],m:[7,13]},
    {r:[2,7],m:[5,11]},{r:[5,4],m:[11,13]},{r:[1,3,5],m:[4,7,9]},{r:[3,8],m:[8,11]}
  ]:[
    {r:[2,3],m:[3,5]},{r:[1,2],m:[4,5]},{r:[2,1],m:[3,5]},{r:[0,3],m:[4,5]},
    {r:[1,3],m:[3,4]},{r:[3,4],m:[5,7]},{r:[2,4],m:[5,7]},{r:[1,5],m:[4,7]}
  ];
...
function startMission(id,{seed=null}={}){
  const mission=missions.find(m=>m.id===id);
  if(!mission || !isMissionUnlocked(mission)){ showToast('Mission locked','Clear the previous operation first.'); return; }
  const suppliedSeed=Boolean(seed);
  const sessionSeed=normalizeSeedCode(seed)||createSessionSeed({type:'M',id:mission.id,difficulty:state.difficulty});
...
function practiceDefinition(type){
  const defs={
    quick:{title:'Quick Practice',story:'Five focused questions with immediate feedback, hints, and explanations.',count:5},
    exam:{title:'Exam Mode',story:'Twenty questions. Hints and explanations are hidden until the final report.',count:20},
    endless:{title:'Endless Mode',story:'Keep solving until you make three mistakes. New unique questions are generated as you advance.',count:12},
    review:{title:'Spaced Review',story:'Review questions that your learning schedule says are due now.',count:10}
  };
  return defs[type]||defs.quick;
}

function startPractice(type='quick',{seed=null,targetSkill=null,reviewKeys=null}={}){
  const def=practiceDefinition(type);
  const suppliedSeed=Boolean(seed);
  const sessionSeed=normalizeSeedCode(seed)||createSessionSeed({type:'P',id:type,difficulty:state.difficulty});
  let questions=[];
  if(type==='review'){
    const records=(reviewKeys?.length?reviewKeys.map(k=>state.mistakes?.[k]).filter(Boolean):dueMistakes(state)).slice(0,def.count);
    questions=records.map(r=>structuredClone(r.question));
    if(!questions.length){ showToast('Review queue clear','Nothing is due right now. Try Quick Practice instead.'); return; }
  }else{
    questions=buildPracticeQuestions(def.count,sessionSeed,{targetSkill,avoidRecent:!suppliedSeed});
  }
  if(!questions.length){ showToast('Practice unavailable','The question engine could not build this session.'); return; }
  registerSessionSeed(state,sessionSeed);
  practiceMode=type;
  currentMission={id:`practice-${type}`,title:def.title,story:def.story,questions:questions.length,baseXp:questions.length*10,kind:'practice',topic:'mixed',label:'Training'};
  missionSession={kind:'practice',practiceType:type,seed:sessionSeed,questions,index:0,earned:0,correct:0,wrong:0,lives:3,batch:0,answered:false,selected:null,hintUsed:false,explainUsed:false,results:[],targetSkill};
  saveState();
  $('#missionKicker').textContent=`Training · ${type==='exam'?'EXAM':type.toUpperCase()}`;
  $('#missionModalTitle').textContent=def.title;
  $('#missionModal').hidden=false;document.body.style.overflow='hidden';
  renderMissionQuestion();
  setTimeout(()=>$('.close-mission')?.focus(),30);
}
...
function reproduceSessionFromSeed(){
  ...
  if(parsed.type==='M'){
    const mission=missions.find(m=>m.id===parsed.id);
    if(!mission){ showToast('Unknown mission','This session code references a mission that is not in this release.'); return; }
    startMission(mission.id,{seed:parsed.code});
    return;
  }
  if(parsed.type==='P' && ['quick','exam','endless'].includes(parsed.id)){
    startPractice(parsed.id,{seed:parsed.code});
    return;
  }
  showToast('Session cannot be reproduced','Review queues depend on changing spaced-repetition state, so only mission, Quick, Exam, and Endless codes are reproducible.');
}

function extendEndlessSession(){
  if(!missionSession || missionSession.practiceType!=='endless') return;
  const existingKeys=new Set(missionSession.questions.map(questionFingerprint));
  missionSession.batch=(missionSession.batch||0)+1;
  const more=buildPracticeQuestions(10,`${missionSession.seed}|B${missionSession.batch}`,{targetSkill:missionSession.targetSkill,avoidRecent:true,existingKeys});
  missionSession.questions.push(...more);
  currentMission.questions=missionSession.questions.length;
}
...
function renderMissionQuestion(){
  ...
  if(index>=questions.length){
    if(missionSession.kind==='practice' && missionSession.practiceType==='endless' && missionSession.lives>0){
      extendEndlessSession();
      if(index<missionSession.questions.length) return renderMissionQuestion();
    }
    if(missionSession.kind==='practice') finishPracticeSession(); else finishMission();
    return;
  }
  ...
  const isEndless=missionSession.practiceType==='endless';
  $('#questionCounter').textContent=isEndless?`Question ${index+1} · Lives ${'♥'.repeat(missionSession.lives)}${'♡'.repeat(Math.max(0,3-missionSession.lives))}`:`Question ${index+1} of ${questions.length}`;
  $('#missionScore').textContent=`${missionSession.earned} XP`;
  $('#missionProgressBar').style.width=isEndless?'100%':`${Math.round(index/questions.length*100)}%`;
  const exam=missionSession.practiceType==='exam';
  $('#hintBtn').hidden=exam; $('#explainBtn').hidden=exam;
  ...
  const seedLine=`<div class="session-code"><span>Session</span><code>${escapeHTML(missionSession.seed)}</code></div>`;
  $('#missionBody').innerHTML=`<div class="mission-story"><strong>Situation:</strong> ${escapeHTML(currentMission.story)}</div>${seedLine}${bossHud}<div class="badge">${escapeHTML(difficulties[state.difficulty].name)} · ${escapeHTML(skills.find(s=>s.id===q.skill)?.name||q.skill)}</div><h3 class="question-prompt">${escapeHTML(q.prompt)}</h3>${answerUI}<div id="missionSupport"></div><div id="missionFeedback"></div>`;
...
function submitMissionAnswer(){
  ...
  missionSession.results.push({correct,given:String(value),prompt:q.prompt,answer:answerLabel(q),skill:q.skill,explanation:q.explanation});
  if(correct){
    const base=missionSession.kind==='practice'?10:(currentMission.baseXp/currentMission.questions);
    ...
    const points=Math.max(5,Math.round(base*multiplier*(1-supportPenalty)));
    missionSession.earned+=points; missionSession.correct++;
    playTone('success');
  }else{
    missionSession.wrong=(missionSession.wrong||0)+1;
    if(missionSession.practiceType==='endless') missionSession.lives=Math.max(0,missionSession.lives-1);
    playTone('error');
  }
  ...
  if(exam){
    feedback.innerHTML=`<div class="feedback"><strong>Answer recorded</strong><br>Exam Mode reveals correctness and explanations only in the final report.</div>`;
  }else{
  ...
  const lastFixed=missionSession.practiceType!=='endless' && missionSession.index===missionSession.questions.length-1;
  const outOfLives=missionSession.practiceType==='endless' && missionSession.lives<=0;
  $('#submitMissionBtn').textContent=outOfLives?'End run':lastFixed?(missionSession.kind==='practice'?'Finish training':'Complete mission'):'Next question →';
  $('#missionScore').textContent=`${missionSession.earned} XP`;
}

function showMissionHint(){
  if(!missionSession || missionSession.answered || missionSession.practiceType==='exam') return;
...
function finishMission(){
  ...
  renderHome(); renderCampaign(); if(currentView==='training')renderTraining();
}

function finishPracticeSession(){
  const s=missionSession,m=currentMission; if(!s||!m) return;
  const total=s.results.length||s.questions.length;
  const accuracy=total?Math.round(s.correct/total*100):0;
  const award=s.earned;
  state.xp+=award;
  recordPracticeRun(state,s.practiceType,s.correct,total);
  saveState();updateAchievements(true);updateGlobalStats();
  if(accuracy>=80) celebrate();
  const wrong=s.results.filter(r=>!r.correct);
  const wrongHtml=wrong.length?`<div class="exam-review"><h4>${s.practiceType==='exam'?'Exam corrections':'Items to review'}</h4>${wrong.slice(0,12).map(r=>`<article><strong>${escapeHTML(r.prompt)}</strong><span>Correct: ${escapeHTML(r.answer)}</span><p>${escapeHTML(r.explanation)}</p></article>`).join('')}</div>`:`<div class="callout success">Perfect run — no mistakes were added to your review queue.</div>`;
  $('#missionProgressBar').style.width='100%';
  $('#questionCounter').textContent=s.practiceType==='endless'?'Run complete':'Training complete';
  $('#missionScore').textContent=`+${award} XP`;
  $('#missionBody').innerHTML=`<div class="mission-complete"><div class="complete-icon">${accuracy>=80?'◇':'✓'}</div><span class="eyebrow">Learning session complete</span><h3>${escapeHTML(m.title)}</h3><p>${s.correct}/${total} correct (${accuracy}%). ${s.practiceType==='endless'?`You survived ${total} questions before losing three lives.`:`You earned ${award} XP.`}</p><div class="session-result-code">Session code: <code>${escapeHTML(s.seed)}</code></div><div class="metrics-grid"><article class="metric-card"><span class="metric-icon">◎</span><div><small>Accuracy</small><strong>${accuracy}%</strong><span>${total} attempted</span></div></article><article class="metric-card"><span class="metric-icon">↻</span><div><small>Due reviews</small><strong>${dueMistakes(state).length}</strong><span>Spaced repetition queue</span></div></article></div>${wrongHtml}</div>`;
  $('#hintBtn').hidden=true;$('#explainBtn').hidden=true;
  $('#submitMissionBtn').textContent='Return to Training';
  $('#submitMissionBtn').onclick=()=>{closeMission();setView('training');};
  missionSession=null;currentMission=null;practiceMode=null;
...
function closeMission(){
  ...
  missionSession=null; currentMission=null; practiceMode=null;
  renderHome(); if(currentView==='campaign') renderCampaign(); if(currentView==='training')renderTraining();
}
...
function renderTraining(){
  ...
  const mistakes=activeMistakes(state);
  const due=dueMistakes(state);
  const recent=state.questionHistory.slice(-60);
  const curated=allStaticQuestionPool().length;
  if($('#trainingDueCount')) $('#trainingDueCount').textContent=String(due.length);
  if($('#recentQuestionCount')) $('#recentQuestionCount').textContent=String(recent.length);
  if($('#activeMistakeCount')) $('#activeMistakeCount').textContent=String(mistakes.length);
  if($('#dueReviewCount')) $('#dueReviewCount').textContent=String(due.length);
  if($('#questionBankCount')) $('#questionBankCount').textContent=String(curated);
  const dueBtn=$('#startDueReview');
  if(dueBtn){dueBtn.disabled=!due.length;dueBtn.textContent=due.length?`Review ${Math.min(due.length,10)} due item${due.length===1?'':'s'}`:'Nothing due now';}

  const notebook=$('#mistakeNotebook');
  if(notebook){
    notebook.innerHTML=mistakes.length?mistakes.slice(0,10).map(r=>{
      const skillName=skills.find(s=>s.id===r.question?.skill)?.name||r.question?.skill||'Unknown skill';
      const label=nextReviewLabel(r);
      return `<article class="mistake-card" data-mistake-key="${escapeHTML(r.key)}"><header><div><span class="eyebrow">${escapeHTML(skillName)}</span><h3>${escapeHTML(r.question?.prompt||'Saved mistake')}</h3></div><span class="badge ${label==='Due now'?'review-due':r.resolved?'review-mastered':''}">${escapeHTML(label)}</span></header><p><strong>Correct answer:</strong> ${escapeHTML(r.answer||answerLabel(r.question))}</p><p>${escapeHTML(r.question?.explanation||'Review the related concept and try again.')}</p><div class="mistake-meta"><span>${r.wrongCount||0} wrong</span><span>${r.correctCount||0} review correct</span><span>Level ${r.level||0}</span></div><div class="mistake-actions"><button class="secondary-btn review-one" data-key="${escapeHTML(r.key)}" type="button">Review this</button><button class="secondary-btn practice-skill" data-skill="${escapeHTML(r.question?.skill||'')}" type="button">Practice similar</button></div></article>`;
    }).join(''):`<div class="training-empty"><strong>No active mistakes yet</strong>Wrong answers will automatically appear here with a spaced-review schedule.</div>`;
    $$('.review-one',notebook).forEach(b=>b.addEventListener('click',()=>startPractice('review',{reviewKeys:[b.dataset.key]})));
    $$('.practice-skill',notebook).forEach(b=>b.addEventListener('click',()=>startPractice('quick',{targetSkill:b.dataset.skill||null})));
  }

  const panel=$('#skillDependencyPanel');
  if(panel){
    const insights=skillDependencyInsights(state,skills);
    const measured=insights.filter(x=>x.attempts>0).sort((a,b)=>{
      const sa=a.status==='needs-work'?0:a.status==='developing'?1:a.status==='strong'?2:3;
      const sb=b.status==='needs-work'?0:b.status==='developing'?1:b.status==='strong'?2:3;
      return sa-sb || a.rate-b.rate;
    }).slice(0,12);
    const foundation=recommendedFoundation(state,skills);
    panel.innerHTML=`${foundation?`<div class="dependency-warning"><strong>Likely prerequisite bottleneck:</strong> ${escapeHTML(foundation.target.name)} is struggling while ${escapeHTML(foundation.dependency.name)} is weak or under-practiced. Strengthen the prerequisite first. <button class="text-btn foundation-practice" data-skill="${escapeHTML(foundation.dependency.id)}" type="button">Train ${escapeHTML(foundation.dependency.name)} →</button></div>`:''}${measured.length?measured.map(x=>{const pct=Math.round(x.rate*100);return `<article class="dependency-card"><div class="dependency-row"><strong>${escapeHTML(x.name)}</strong><div class="dependency-track"><span style="width:${pct}%"></span></div><span>${pct}%</span></div><div class="dependency-prereq">Prerequisites: ${x.dependencyNames.length?escapeHTML(x.dependencyNames.join(' → ')):'Foundation skill'}${x.weakDependencyNames.length?` · <strong>Review:</strong> ${escapeHTML(x.weakDependencyNames.join(', '))}`:''}</div></article>`;}).join(''):`<div class="training-empty"><strong>No skill diagnosis yet</strong>Answer a few mission or practice questions and Cryptic Quest will map likely prerequisite weaknesses.</div>`}`;
    $('.foundation-practice',panel)?.addEventListener('click',e=>startPractice('quick',{targetSkill:e.currentTarget.dataset.skill}));
  }

  const seeds=$('#recentSeeds');
  if(seeds){
    seeds.innerHTML=state.sessionSeeds.length?`<span class="eyebrow">Recent codes</span>${state.sessionSeeds.slice(0,8).map(x=>`<button class="seed-chip" type="button" data-seed="${escapeHTML(x.code)}">${escapeHTML(x.code)}</button>`).join('')}`:`<span class="training-empty">Session codes will appear after you start a mission or practice run.</span>`;
    $$('.seed-chip',seeds).forEach(b=>b.addEventListener('click',()=>{if($('#seedReplayInput'))$('#seedReplayInput').value=b.dataset.seed;}));
  }
}
...
function sanitizeImportedState(data){
  ...
  if(data.daily&&typeof data.daily==='object')fresh.daily={...fresh.daily,...data.daily,solvedDates:Array.isArray(data.daily.solvedDates)?data.daily.solvedDates:[]};
  fresh.questionHistory=Array.isArray(data.questionHistory)?data.questionHistory.slice(-200):[];fresh.mistakes=data.mistakes&&typeof data.mistakes==='object'&&!Array.isArray(data.mistakes)?data.mistakes:{};fresh.sessionSeeds=Array.isArray(data.sessionSeeds)?data.sessionSeeds.slice(0,20):[];fresh.practiceStats=data.practiceStats&&typeof data.practiceStats==='object'?data.practiceStats:fresh.practiceStats;return ensureLearningState(fresh);
}
...
function resetProgress(){
  if(!confirm('Reset all Cryptic Quest progress stored in this browser? This cannot be undone unless you exported it first.'))return;localStorage.removeItem(STORAGE_KEY);state=ensureLearningState(defaultState());academyDepth=state.explanationDepth;saveState();closeProfile();renderAll();showOnboarding();showToast('Progress reset','A fresh local profile has been created.');
}
...
function renderAll(){updateGlobalStats();renderHome();renderCampaign();renderAcademy();renderGuide();renderTraining();if(currentView==='sandbox')renderSandbox(activeSandbox);if(currentView==='attack')renderAttack(activeAttack);if(currentView==='challenges')renderChallenges(activeChallenge);$('#soundToggle').textContent=state.sound?'🔊':'🔇';}
...
function bindEvents(){
  ...
  $('#startQuickPractice')?.addEventListener('click',()=>startPractice('quick'));$('#startExamPractice')?.addEventListener('click',()=>startPractice('exam'));$('#startEndlessPractice')?.addEventListener('click',()=>startPractice('endless'));$('#startDueReview')?.addEventListener('click',()=>startPractice('review'));$('#replaySeedBtn')?.addEventListener('click',reproduceSessionFromSeed);$('#seedReplayInput')?.addEventListener('keydown',e=>{if(e.key==='Enter')reproduceSessionFromSeed();});
